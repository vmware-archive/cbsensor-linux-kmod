...
//
//  To support ENT-1329, we are trying to determine the type of the file from
//  the existence of a matching "magic" number.  Would like to revisit this at
//  some point to create a more scalable approach (in the likely case that we
//  add more types)!  Something along the line of a tree-walk maybe.
//
//
//  We are splitting up the EICAR test signature to prevent virus scanners from
//  detecting it in our image.
//
static const char *EICAR_TEST_FILE_2 =
	"!P%@AP[4\\PZX54(P^)7CC)7}$EICAR-STANDARD-ANTIVIRUS-TEST-FILE!$H+H*";
static const char *EICAR_TEST_FILE_1 = "X5O";

/*
 * @brief queried a file write window to look for the existence of a file that
 * we are interestd in
 *
 * @param[in] currentData - the current callback data from the write
 * notification
 * @param[inout] pFileType - a pointer to a dword that will contain the file
 * type
 * @param[in] determineDataFiles - whether or not to try to determine data file
 * types
 *
			 ...
			 enum CB_FILE_TYPE *pFileType, bool determineDataFiles)
{
	...
	// sizeof e_ident buffer + e_type
	if (bytes_read >= offsetof(Elf32_Ehdr, e_machine) &&
	    ELFCLASS32 == buffer[EI_CLASS]) {
	...
	// sizeof e_ident buffer + e_type
	else if (bytes_read >= offsetof(Elf64_Ehdr, e_machine) &&
		 ELFCLASS64 == buffer[EI_CLASS]) {
	...
	if (bytes_read >= MAX_FILE_BYTES_TO_DETERMINE_TYPE) {
		if (memcmp(EICAR_TEST_FILE_1, buffer, 3) == 0) {
			PR_DEBUG("Found EICAR PRE");
			if (memcmp(EICAR_TEST_FILE_2, buffer + 3,
				   MAX_FILE_BYTES_TO_DETERMINE_TYPE - 3) == 0) {
	...
	if (determineDataFiles) {
		...
		if (bytes_read >= 4) {
			...
			if (memcmp("\x50\x4B\x03\x04", buffer, 4) == 0) {
				if (bytes_read >= 8 &&
				    memcmp("\x50\x4B\x03\x04\x14\x00\x06\x00",
					   buffer, 8) == 0) {
		...
		if (bytes_read >= 6) {
			if (memcmp("\x37\x7A\xBC\xAF\x27\x1C", buffer, 6) ==
			    0) {
		...
		if (bytes_read >= 7) {
			if (memcmp("\x52\x61\x72\x21\x1A\x07\x00", buffer, 7) ==
			    0) {
