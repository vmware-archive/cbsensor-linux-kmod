				     ...
				     unsigned int flags)
{
	...
	/* strncpy_from_user() does an access_ok check to see if this is user
	   ...
	   we'll assume that it is already kernel memory.
	 */
	...
	if (strncmp(name_kernel, DRIVER_NAME, slen) != 0) {
	...
	/* If the syscall or lsm function hooks have changed since we set them,
	   ...
	   call our (non-existent) hooks. For that reason, we say we are busy
	   and refuse to unload. We also remove our device node in case Cb
	   Response with an incompatible version of the daemon.
	   TODO: Add a version compatibility check between the daemon and the
